...
let dia_salida = `${fechaActual('dia')}_salida`
let t_asistencias = `asistencias_${fechaActual('anio')}_w${fechaActual('semana')}`
  ...
  conexion.conexion.query('select * from formato_auth where estado = "activo"',(error,result)=>{
    if(error){
      ...
      conexion.conexion.query(`SELECT f1.numero_empleado, f1.punto, f1.puesto, el.nombres, el.apellido_paterno, el.apellido_materno, f1.${fechaActual('dia')} as dia, a.${dia_entrada} as dia_entrada, a.${dia_salida} as dia_salida FROM ${f_activo}  AS f1 INNER JOIN elementos AS el ON f1.numero_empleado = el.numero_empleado INNER JOIN ${t_asistencias} AS a ON f1.numero_empleado = a.numero_empleado `,(error1,result1)=>{
        if(error1){
          console.log(error1)
          res.send({
            mensaje: 'ocurrio errror en cosultar el estado de los formatos'
